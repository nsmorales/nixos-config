...
in
{
  # This module is imported by each platform's home-manager configuration
  programs = {
    ...
    ssh = {
      ...
      # "*" applies to all hosts; add per-host blocks as needed.
      # Option names are upstream ssh_config(5) directives.
      settings = {
        "*" = {
          SendEnv = [ "LANG" "LC_*" ];
          HashKnownHosts = true;
        };
        # Example per-host configuration for GitHub
        # "github.com" = {
        #   IdentitiesOnly = true;
        #   IdentityFile = [
        #     (lib.mkIf pkgs.stdenv.hostPlatform.isLinux
